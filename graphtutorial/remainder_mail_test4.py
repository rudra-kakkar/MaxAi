...
def get_pending_emails():
    """Fetch emails with expected_date less than or equal to today's date."""
    conn = connect_db()
    cursor = conn.cursor(dictionary=True)
    
    cursor.execute("SELECT user_name, email, subject, expected_date, frequency FROM emails WHERE expected_date <= %s", (date.today(),))
    emails = cursor.fetchall()
    
    conn.close()
    return emails

def update_expected_date(email, frequency):
    """Update the expected_date for recurring emails."""
    if frequency:
        new_expected_date = date.today() + timedelta(days=frequency)
        query = """
            UPDATE emails 
            SET expected_date = %s 
            WHERE email = %s
...
async def check_emails_for_subjects(graph: Graph, pending_emails):
    ...
    if message_page and message_page.value:
        for message in message_page.value:
            if message.from_ and message.from_.email_address and message.subject:
    ...
    for user in pending_emails:
        ...
        # Ensure expected_date is a date object
        if isinstance(expected_date, str):
            expected_date = date.fromisoformat(expected_date)

        delay_days = (date.today() - expected_date).days  # Calculate delay on the fly

        if email not in users_who_sent_emails:
            users_to_notify.append({**user, "delay_days": delay_days})
        else:
            update_expected_date(email, frequency=user["frequency"])

    return users_to_notify

async def send_reminder(graph: Graph, user):
    """Send a reminder email if the delay is 0 days."""
    subject = f"Reminder: Send Your Report - {date.today()}"
    prompt = f"Dear {user['user_name']}, this is a reminder to send your required report today."
...
async def send_alert(graph: Graph, user):
    ...
    prompt = f"URGENT: {user['user_name']} has not submitted the required report for {delay_days} days. Immediate action needed!"
    ...
    while True:
        try:
            ...
            if users_to_notify:
                ...
                print(f"✅ All emails received on time for {date.today()}")
